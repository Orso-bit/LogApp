...
struct HeightMeasurementForNewTreeView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            .sheet(isPresented: $showingResult) {
                SaveNewTreeMeasurementView(
                    height: measuredHeight,
                    notes: $measurementNotes,
                    treeName: treeName,
                    treeSpecies: treeSpecies,
                    onSave: { height, notes in
                        saveTreeWithMeasurement(height: height, notes: notes)
                    }
                )
            }
        }
    ...
    private func saveTreeWithMeasurement(height: Double, notes: String) {
        ...
        // Crea la misurazione
        let measurement = Measurement(height: height, notes: notes, tree: newTree)
        
        // Aggiungi la misurazione all'albero
        newTree.measurements.append(measurement)
...
struct SaveNewTreeMeasurementView: View {
    @Environment(\.dismiss) private var dismiss
